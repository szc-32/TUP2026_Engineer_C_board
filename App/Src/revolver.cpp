...
/********电机初始化设置*********/
Motor_Setting_t firc_l_config = {"fir_l",ON_CAN1,1,M3508,NEGATIVE_DIRECT,DIRECT_DRIVE,SINGLE_LOOP};
Motor_Setting_t firc_r_config = {"fir_r",ON_CAN1,2,M3508,POSITIVE_DIRECT,DIRECT_DRIVE,SINGLE_LOOP};
Motor_Setting_t stir_config = {"stir",ON_CAN1,6,AM3508,POSITIVE_DIRECT,RATIO_1_TO_71,CASCADE_LOOP};
...
//	
//	revolver.revolver_msg.if_stuck=0;
//}
/**
  * @brief          发射机构主任务
  * @param[in]      NULL
...
void Revolver_t::InfoUpdate()
{
	...
	//剩余热量=上限-当前热量
	revolver_msg.rest_heat = JUDGE_usGetHeatLimit_17mm() - JUDGE_usGetHeatLimit_17mm(); 
	...
	//判断是否达到误差角内（需改0）
	if(fabs(stir_angle_set - stir_angle)<AN_BULLET*0.2f)
	...
	/*****拨盘反转逻辑判断*****/
//	//old
//	if(SysPointer()->stir_mode==SHOOT_FIXED_FIRE_RATE)
//	{
//	    if(fabs(stir_speed_set-stir_speed)>2.0f)
//		{
//			 unfinish_time++;
//		}else
//		{
//			unfinish_time=0;
////			revolver_msg.if_stuck=0;
//		}
//		if(unfinish_time>100)
//		{
//			revolver_msg.if_stuck=1;
//		}
//	}else if(SysPointer()->stir_mode==SHOOT_1_BULLET)
//	{
//	    if(fabs(stir_angle_set - stir_angle)> AN_BULLET)
//		{
//			 unfinish_time++;
//		}else
//		{
...
//    }
	...
	{
		//此时改为速度单环
		stir_motor_gun[MAIN_REVOLVER].ChangeControlMode(SINGLE_LOOP);
		//控制器参数重置——仅需一次即可（若角度闭环时的速度环效果较好可以直接用，即省略控制器参数重置）
			 ...
			 (last_stir_mode != REVERSE_MOVE && SysPointer()->stir_mode == REVERSE_MOVE))
	{
...
void Revolver_t::FircSpeedSet()
{
	...
	DifferentFirSpeedSet(LOW_SPEED);
	l_firc_ramp_set[FIRST_STAGE] = RAMP_float(l_firc_speed_set[FIRST_STAGE],l_firc_ramp_set[FIRST_STAGE],10);
	r_firc_ramp_set[FIRST_STAGE] = RAMP_float(r_firc_speed_set[FIRST_STAGE],r_firc_ramp_set[FIRST_STAGE],10);
#endif
}
