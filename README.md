...
**Bsp层**内文件为板级支持包，在其中我们封装了各类HAL库的函数，与HAL库耦合，若需移植到其他开发板上，无需更改Bsp层。在bsp_usart中我们通过预编译，对不同开发板的串口直接进行了不同的定义（仅针对Robomaser开发板C型和A型），其余开发板若有不同需更改，其使用了注册登记的形式，当需要接收信息时仅需简单注册登记并编写解码函数即可快速实现串口通信。

在bsp_can中，我们为了提高开发效率，也使用了注册登记表的形式，当有新的CAN线设备注册时，无需再在CAN接收回调函数中进行改动代码，仅需注册登记并编写解码函数即可直接获取信息，封装为新的使用CAN线仅需通信的模块十分方便（如电机、超级电容），也可直接使用用于自定义CAN信息的解码，简化了开发流程。
...
我们对电机着重进行了封装，将**控制器**选择封装在电机类内，在有需求增加新电机时可快速进行添加，无需在多个地方进行更改，有需求添加新控制器时对电机类扩展也较为便捷，同时提供了许多获取电机运行状态的API，如获取输出轴转速、转子转速、累积角度等等；同时也对其进行了**控制器是否初始化**和**电机温度**的监测，可在代码编写以及程序正常运行时对电机状态进行**可视化**呈现，由于是通过**回调函数**进行实现，后续如有对电机其他指标进行监测的需求也可**快速扩展**。
...
### AS5600L六轴自定义控制器通信协议（建议统一）

本工程当前链路：**串口接收6路编码器原始值 -> 正解算末端位姿 -> 逆解算关节角 -> 达妙电机MIT执行**。  
为避免上位机与下位机字段错位，建议按以下固定协议打包。

#### 1) 数据帧载荷定义（DATA区）

| 字段 | 字节偏移 | 长度 | 类型 | 说明 |
|---|---:|---:|---|---|
| j1_raw12 | 0 | 2 | uint16_le | 关节1 AS5600L原始值，低12位有效 |
| j2_raw12 | 2 | 2 | uint16_le | 关节2 AS5600L原始值，低12位有效 |
| j3_raw12 | 4 | 2 | uint16_le | 关节3 AS5600L原始值，低12位有效 |
| j4_raw12 | 6 | 2 | uint16_le | 关节4 AS5600L原始值，低12位有效 |
| j5_raw12 | 8 | 2 | uint16_le | 关节5 AS5600L原始值，低12位有效 |
| j6_raw12 | 10 | 2 | uint16_le | 关节6 AS5600L原始值，低12位有效 |

- DATA长度最小为 `12` 字节（6轴 × 2字节）。
- 字节序为小端：`raw = low_byte | (high_byte << 8)`。
- 接收端统一掩码：`raw12 = raw & 0x0FFF`。

#### 2) 上位机发包约束

- 六轴顺序固定为 `J1 -> J2 -> J3 -> J4 -> J5 -> J6`，不可调换。
- 每轴范围建议约束在 `[0, 4095]`。
- 推荐发送频率 `50~200Hz`（与机械臂控制周期匹配）。
- 如你们后续要扩展附加字段（按键/摇杆/姿态），请放在第12字节之后，且前12字节保持不变。

#### 3) 示例（仅DATA区12字节）

若6轴原始值依次为：

- J1=`0x0123` J2=`0x0456` J3=`0x0789` J4=`0x0ABC` J5=`0x00DE` J6=`0x0F01`

则DATA按小端打包为：

- `23 01 56 04 89 07 BC 0A DE 00 01 0F`

#### 4) C板侧处理链路（当前实现）

1. 串口帧解析到自定义交互命令后，读取DATA前12字节为6路 `raw12`。  
2. `raw12` 经每轴零位、比例、方向换算为关节角，执行正解算得到末端位姿。  
3. 将末端位姿写入机械臂目标位姿，同时将当前关节角作为IK初值。  
4. 机械臂控制任务执行逆解算并输出MIT命令，通过CAN下发至六轴电机。

> 说明：如果后续要兼容其他磁编码器，请保持该12字节主载荷不变，仅在上位机侧做原始值映射。
