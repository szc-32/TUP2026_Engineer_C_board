...
Dji_Motor_Measure_t::Dji_Motor_Measure_t()
{
	ecd = 0xFFFFU;           // 初始化编码器值为无效哨兵值
    speed_rpm = 0;           // 初始化电机转速(rpm)为0
    given_current = 0;       // 初始化给定电流为0
    temperate = 0;           // 初始化温度为0
    last_ecd = 0;            // 初始化上一次编码器值为0
    difference_num = 0;      // 初始化编码器圈数计数为0
	output_encoder_num = 0;  // 初始化输出轴编码器圈数为0
}


void Dji_Motor_Measure_t::Dji_Motor_Measure_init()
{
	ecd = 0;
	speed_rpm = 0;
	given_current = 0;
	...
	last_ecd = 0;
	difference_num = 0;
	output_encoder_num = 0;
	output_encoder_ecd = 0;
...
fp32 DJIMotorInstance::FilterSpeed(fp32 *fliter_num)
{
	...
	}else
	{
		...
		motor_measure.speed_fliter_3 = motor_measure.speed_fliter_2 * fliter_num[0] + motor_measure.speed_fliter_1 * fliter_num[1] + (motor_measure.speed_rpm * RPM_TO_RAD_S * motor_measure.reduction_ratio) * fliter_num[2];
...
void DJIMotorInstance::MonitorCurrent()                          //电流监测（电机堵转） 
{                                                                //添加
//	if (abs(chassis_motor[0].motor_measure.given_current) > 500)
//	{
//		SysPointer()->mode = ZERO_FORCE;
//	}
