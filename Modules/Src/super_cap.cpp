...
//浙纺超电
#define get_tianxu_cap_measure(ptr, data)			\
{																									\
	(ptr)->Cap_input_vol = (uint16_t)((data[0] << 8) | (data[1])) * 0.01f;           \
	(ptr)->Cap_input_current = (uint16_t)((data[2] << 8) | (data[3])) * 0.01f;     \
	(ptr)->Cap_voltage = (uint16_t)((data[4] << 8) | (data[5])) * 0.01f; \
	(ptr)->Cap_power = (uint16_t)((data[6] << 8) | (data[7])) * 0.01f; \
}
...
#elif SUPER_CAP_TYPE == TIANXUCAP	
	TianXu_Cap_Measure_t *measure = &cap->tianxu_cap;
	get_tianxu_cap_measure(measure,rxbuff);
#endif
}
...
void Cap_t::EncodeControlData(uint8_t dischargecmd,uint8_t ctrlmode,uint16_t chassis_power_buff,uint16_t chassis_power_limit,uint16_t chassis_power_buff_limit)
{
	send_data[0] = dischargecmd;//是否开超电
    send_data[1] = ctrlmode;//控制模式 默认0
    send_data[2] = chassis_power_buff >> 8;
    send_data[3] = chassis_power_buff;//实际缓冲能量
    send_data[4] = chassis_power_limit >> 8;
    send_data[5] = chassis_power_limit;//功率上限
    send_data[6] = chassis_power_buff_limit >> 8;
    send_data[7] = chassis_power_buff_limit;//缓冲能量上限
}

void Cap_t::EncodeControlData(uint16_t chassis_power_limit)
{
	send_data[0] = chassis_power_limit >> 8;
	send_data[1] = chassis_power_limit;
}
