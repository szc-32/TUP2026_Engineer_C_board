...
static bool_t g_custom_controller_updated = FALSE;
static void RefereeTryFeedMasterRaw12AS5600L(const uint8_t *data_ptr, uint16_t data_len);
...
bool Judge_Read_Remotor_Data(uint8_t *ReadFormUsart1)
{
//   	bool retval_tf = FALSE;//数据正确与否标志,每次调用读取裁判系统数据函数都先默认为错误
//	
   uint16_t judge_length;//统计一帧数据长度 
...
//	/***------------------*****/
//	//无数据包，则不作任何处理
//	if (ReadFormUsart1 == NULL)
//	{
//		return -1;
//	}
//	
//	//写入帧头数据,用于判断是否开始存储裁判数据
//	memcpy(&RemotorFrameHeader, ReadFormUsart1, LEN_HEADER);
//		//判断帧头数据是否为0xA5
	for(int i=0;i<80;i++)
	{
   if(ReadFormUsart1[ i ] == JUDGE_FRAME_HEADER)
   {
	   //帧头CRC8校验
	   if (Verify_CRC8_Check_Sum( ReadFormUsart1+i, LEN_HEADER ) == TRUE)
	   {
		   //统计一帧数据长度,用于CR16校验
		   judge_length = ReadFormUsart1[ DATA_LENGTH+i ] + LEN_HEADER + LEN_CMDID + LEN_TAIL;
		   
		   //帧尾CRC16校验
		   if(Verify_CRC16_Check_Sum(ReadFormUsart1+i,judge_length) == TRUE)
		   {
			   //retval_tf = TRUE;//都校验过了则说明数据可用
			   ...
			   //首地址加帧长度,指向CRC16下一字节,用来判断是否为0xA5,用来判断一个数据包是否有多帧数据
//				if(*(ReadFormUsart1+i + sizeof(xFrameHeader) + LEN_CMDID + RemotorFrameHeader.DataLength + LEN_TAIL) == 0xA5)
//				{
//					//如果一个数据包出现了多帧数据,则再次读取
//					Judge_Read_Data(ReadFormUsart1 + sizeof(xFrameHeader) + LEN_CMDID + RemotorFrameHeader.DataLength + LEN_TAIL);
//				}
		   }					
	   }		
   }
...
bool_t RefereePopCustomControllerData(ext_robot_interactive_data_t *data_out)
{
	...
	if (g_custom_controller_updated == FALSE)
	{
		if (primask == 0U)
		{
			__enable_irq();
	...
	*data_out = robot_interactive_data;
	g_custom_controller_updated = FALSE;
...
bool_t RefereeGetLatestCustomControllerData(ext_robot_interactive_data_t *data_out)
{
	uint32_t primask = __get_PRIMASK();
	__disable_irq();
...
static void RefereeTryFeedMasterRaw12AS5600L(const uint8_t *data_ptr, uint16_t data_len)
{
	...
	if (data_len < (uint16_t)CUSTOM_CTRL_AS5600L_PAYLOAD_LEN)
	{
		return;
	}

	uint16_t raw12[CUSTOM_CTRL_AS5600L_AXIS_NUM];
	for (uint8_t i = 0; i < CUSTOM_CTRL_AS5600L_AXIS_NUM; i++)
	{
		uint16_t raw = (uint16_t)data_ptr[i * CUSTOM_CTRL_AS5600L_BYTES_PER_AXIS]
			| ((uint16_t)data_ptr[i * CUSTOM_CTRL_AS5600L_BYTES_PER_AXIS + 1U] << 8);
		raw12[i] = raw & FK_AS5600L_RAW_MASK;
	}

	DMArmMasterSetEncoderRaw12AS5600L(raw12);
}
...
/**
  ...
  * @attention  
  */
uint16_t JUDGE_usGetHeatLimit_17mm(void)
{
...
//uint16_t JUDGE_usGetShoot42mm_speed_limit(void)
//{
//	return Robot_status.shooter_id1_42mm_speed_limit;
//}
...
{
	return 	Robot_status.power_management_shooter_output;
}
...
/**
  ...
  * @attention  
  */
uint8_t JUDGE_usGetJudge_gimbal_Power(void)
{
	return 	Robot_status.power_management_gimbal_output;
}
...
{
	if(Robot_status. current_HP == 0 && JUDGE_sGetDataState() == TRUE)
	{
		return TRUE;
	}
	else
