...
extern "C" volatile uint32_t g_j2_disable_tx_cnt_dbg = 0;
extern "C" volatile uint32_t g_j2_mit_tx_cnt_dbg = 0;
/*
...
void disable_motor_mode(hcan_t* hcan, uint16_t motor_id, uint16_t mode_id)
{
	...
	if ((mode_id == MIT_MODE) && (motor_id == 0x02U))
	{
		g_j2_disable_tx_cnt_dbg++;
	}
...
{
	if ((motor != 0) && (motor->id == 0x02U) && (motor->dm_ctrl_set.mode == mit_mode))
	{
		g_j2_mit_tx_cnt_dbg++;
	}
...
void DMMotorInstance::DM_motor_para_init(DM_motor_t *motor)//在控制器参数中调用的
{
			...
			motor->dm_ctrl_set.pos_set 	= INIT_YAW_SET;//初始位置设定
			...
			//disable_motor_mode(dm_hcan, dm_motor[Motor1].id, MIT_MODE );
